...
class FaceAppBackend:
    ...
    def _train_recognizer(self):
        ...
        # Rebuild label_map to ensure it's dense and correctly reflects trained labels
        # This is important if some label_ids were skipped or re-assigned.
        # It's safer to build a reverse map after training to avoid issues.
        final_label_map = {idx: identity for idx, identity in enumerate(sorted(label_map.values(), key=lambda x: x[1]))}
        # The above line sorts by emp_id for consistent label assignment if labels are recreated.
        # However, LBPH assigns labels sequentially based on the order of unique identities found during training.
        # So, the original approach of `label_map[label_id] = current_identity` and then mapping `labels.append(label_id)` is more direct.
        # Let's stick to the original mapping logic during training and ensure `predict` works with it.
        # The key is that `label_map` must correctly map the integer ID from `predict` back to name/emp_id.
    ...
    def _save_email(self, emp_id: str, email: str) -> None:
        # Use a lock if user_emails could be modified by multiple threads simultaneously
        # For this application, it's likely single-threaded for registration, but good practice.
    ...
    def _handle_successful_recognition(self, name: str, emp_id: str, face_roi_color: np.ndarray) -> None:
        ...
        # Check if this is the first recognition for this user today
        # Ensure thread-safe access if this dict could be updated by multiple concurrent recognitions
        with threading.Lock(): # Added lock for daily_first_recognition access
        ...
        if user_email:
            if is_in_time:
                # Ensure thread-safe access for daily_attendance_status
                with threading.Lock():
        ...
        # Ensure thread-safe update for last_recognized_info
        with threading.Lock():
    ...
    def process_frame(self, frame_data_b64: str) -> Dict[str, Any]:
        try:
            ...
            for (x, y, w_s, h_s) in faces:
                ...
                if self.capture_mode:
                    with self.capture_lock:
                        ...
                        if self.capture_collected_count >= self.capture_target_count:
                            ...
                            # Retrain in a separate thread to avoid blocking the API response
                ...
                # LBPH confidence: lower values mean higher confidence (closer match)
                # A common threshold for LBPH is around 50-70. Adjust as needed.
                # If confidence is below the threshold, it's a known face.
                if conf < 60: # Threshold for known faces (adjust this value if needed)
                    known_face_detected_in_frame = True
                    now = time.time()
                    # Ensure thread-safe access to last_seen_time
                    with threading.Lock():
            ...
            if not known_face_detected_in_frame and len(faces) > 0: 
                ...
                with threading.Lock(): # Ensure thread-safe access to last_unknown_greeting_time
    ...
    def _retrain_after_capture(self):
        # This function should ensure it gets the latest label_map and recognizer
        self.recognizer, self.label_map = self._train_recognizer()
    ...
    def start_capture_samples(self, name: str, emp_id: str, updating=False, sample_count=None) -> Dict[str, Any]:
        with self.capture_lock:
            ...
            if updating:
                # When updating, try to find the existing name from label_map if 'name' is not provided
            ...
            # Determine the starting index for new samples to avoid overwriting existing ones
    ...
    def get_user_email(self, emp_id: str) -> Dict[str, Any]:
        ...
        # Find name from label_map by emp_id
    ...
    def send_otp_flow(self, emp_id: str, email: str, name: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Use a lock for otp_storage and pending_names as they are modified
        with threading.Lock():
    ...
    def verify_otp(self, emp_id: str, otp_entered: str) -> Dict[str, Any]:
        # Use a lock for otp_storage access
        with threading.Lock():
    ...
    def get_last_recognized_info(self) -> Dict[str, Any]:
        # Use a lock for last_recognized_info access
        with threading.Lock():
            info = self.last_recognized_info
            if info:
                self.last_recognized_info = {} # Clear after retrieval
...
# This is useful for testing or ensuring a report is generated soon after deployment
scheduler.add_job(
...
# Schedule subsequent reports for the first day of every month at 1 AM.
# This is a more robust way to get "end of month" data as it covers all month lengths.
# The report generation logic will then look for *previous* month's data.
scheduler.add_job(
    backend_instance.generate_and_send_monthly_reports,
    "cron",
...
def register_user_endpoint():
    ...
    def registration_tts_sequence():
        ...
        time.sleep(0.005) # Small delay to ensure audio streams don't overlap too much
        face_app_backend.tts_helper.speak("I am clicking your photo... 3.... 2.... 1...Smaaaaaail")

    def tts_and_capture():
        registration_tts_sequence()
        # Start capture with the specified sample_count
...
def start_update_capture_endpoint():
    data = request.json
    name = data.get("name") # Name is optional for update, but good to have if known
    emp_id = data.get("emp_id")
...
if __name__ == "__main__":
    # Suppress Flask scheduler exits on reload during development
    # In production, debug=False is recommended.
    try:
        app.run(host="0.0.0.0", port=5000, debug=True, use_reloader=False) # use_reloader=False to prevent scheduler duplication
